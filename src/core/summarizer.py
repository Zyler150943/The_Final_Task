...
class TextSummarizer:
    ...
    def detect_language(self, text: str, use_advanced: bool = True) -> str:
        ...
        result = self.language_detector.detect_language(
            text, prefer_fasttext=use_advanced
        )
    ...
    def _get_or_load_model(self, language_code: str) -> AbstractiveModel:
        ...
        try:
            model = ModelFactory.create_model(
                language=language_code, device=self.device
            )
    ...
    def _calculate_summary_lengths(
        self, text: str, language: str, compression: float
    ) -> Dict[str, int]:
        """
    ...
    def _preprocess_for_summarization(
        self, text: str, language: str, max_input_length: int
    ) -> str:
        """
        ...
        if len(cleaned_text.split()) > max_input_length:
            ...
            parts = self.text_processor.split_long_text(
                cleaned_text, max_tokens=max_input_length
            )
    ...
    def _extract_key_points(
        self, text: str, language: str, num_points: int = 5
    ) -> List[str]:
        """
        ...
        try:
            ...
            try:
                ...
                return [
                    f"{i+1}. {sentences[i]}"
                    for i in range(min(num_points, len(sentences)))
                ]
            except Exception:
    ...
    def summarize(
        ...
        if language_code not in self.LANGUAGE_CONFIGS:
            raise ValueError(
                f"Резюмирование на языке '{language_code}' не поддерживается"
            )
        ...
        preprocessed_text = self._preprocess_for_summarization(
            text, language_code, lengths["max_input_length"]
        )
        ...
        if abstractive:
            ...
            summary = self._extractive_summarize(
                preprocessed_text, language_code, compression
            )
    ...
    def _extractive_summarize(
        ...
        try:
            ...
            logger.info(
                f"Экстрактивное резюмирование: выбрано {num_sentences}/{len(sentences)} предложений"
            )
